<template lang="pug">
  .withdraw
    ...
    div
      div 我的可提现金额：
      span.money  {{totalBalance}}
    div
      div   收款银行：
      el-input(
          class="input",v-model="withdrawData.bank_name", placeholder="输入银行"
        )

      .remind(v-popover:popover="")    已存银行 
          .trangle
          el-popover(
              ref="popover",
            ...
            .wr-list
              ...
              .wri-name   {{bankCard.holder_name}}
              .wri-code   {{`${bankCard.bank_name}(${bankCard.bank_card_number})`}}

    div
        div 开户支行：
        el-input(class="input",v-model="withdrawData.bank_branch_name", placeholder="输入完整开户行信息") 
        span.branch-remind  例：中国银行北京通州九棵树支行
    div
        div 收款卡号：
        el-input(class="input",v-model="withdrawData.bank_card_number", placeholder="输入卡号")
    div 
        div 开户人姓名：
        el-input(class="input",v-model="withdrawData.holder_name", placeholder="输入开户人姓名") 
    div
        div 提现金额：
        el-input(class="input",autofocus, v-model="inputMoney", placeholder="输入提现金额") 
    div
        div 到账时间：
        el-radio(class="radio", v-model="withdrawData.arrive_delay", label="normal")    普通（3-5个工作日）
        el-radio(class="radio",v-model="withdrawData.arrive_delay", label="urgent")     加急（1-2个工作日）
    div
        div 服务费：
        span    ¥{{serviceMoney}}(付款总额
            span.money  ¥{{totalMoney}}）
    div
        div 提现密码：
        el-input(class="input",type="password", v-model="withdrawData.password", placeholder="输入提现密码")
        span(class="remind",@click="passwordShow = true")   设置提现密码
    div
        div
        el-button(class="submit",type="primary" @click="submit")  提交申请 
    div(class="password-container",v-if="passwordShow")
      Password(
        @close="closePassword",
        :title="pTitle",
        :action="pAction"
    ...
    .w-remind
      h4    申请提现遇到问题？
      ul
        li  1.转账到银行卡信息没填对，资金如何退还？
        li  答：款项会全额退还。 本金、服务费将退回萤火虫账户余额里，请关注。 银行退款一般会在7个工作日内退回（处理速度由银行决定）。 
        li  2.服务费收费标准?
        li  答：普通（3-5个工作日）到账，费率0.15%（最低2元，最高25元）；加急（1-2个工作日）到账，费率0.2%（最低2元，最高25元）。
        li  3.提现限额标准？
        li  答.单笔：5万元（向个人账户）；10万元（向公司账户）。
