...
.account
    ...
    div 登录密码：
        span    {{userInfo.pwd}}
        span.util(@click="showPassword")    修改登录密码
    div 提现密码：
        span    {{userInfo.pays_pwd}}
        span.util(@click="showPayPassword") 设置提现密码
    div 用户姓名：
        span    {{userInfo.full_name}}
    div 公司名称：
        span    {{userInfo.company}}
    div 所在城市：
        span    {{userInfo.area_cn}}
    div 公司类型：
        span    {{userInfo.company_type}}
    
    div 营业执照： 
        div(class="util uploaded", v-if="userInfo.has_license")
        span(class="util uploading", v-if="isUploading") 正在上传...
        span.util(@click="preLoad", v-if="!userInfo.has_license && !isUploading") 立即上传
        input(
            ref='file',
            class="file",
            type="file",
    ...
    .password-container(v-if="passwordShow")
