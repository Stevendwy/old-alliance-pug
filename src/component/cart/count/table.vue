<template lang="pug">
  .table
    .header
      .h-item(v-for="(item, index) in headerTitles", :key="index")
          .hi-first( v-if="index === 0")
              el-checkbox(
                  v-model="allSelected",
                  @change="selectedAll"
              )   全选
      ...
      .content(v-if="orders.length")
        .c-container(
          ...
          el-checkbox(
              ...
              @change="selectedPart(order, $event)"
          )   {{order.title}}
          div(
              :class="enable(item) ? 'c-items' : 'c-items-disable'",
              v-for="(item, gIndex) in order.items",
              :key="gIndex"
          )
              .c-item
                  el-checkbox(
              ...
              .c-item
                  el-input-number(
                      ...
                      @change="inputNumberChange($event, item)",
                      :min="1",
                      :max="enable(item) ? 999 : 1",
                      label="描述文字"
                    ) 
              
              .c-item {{(item.price * item.quantity).toFixed(2)}}

              div(
                class="c-item c-delete",
                @click="deleteSub({p_uid: item.p_uid})"
                )    删除
