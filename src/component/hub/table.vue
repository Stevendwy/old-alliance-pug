...
.hub-result-container
    .hub-filter
        .hub-filter-left
            ...
            el-button.filter-btn(size="small", @click="handleTypeToDo('delete')") 删除
            el-button.filter-btn(size="small", @click="handleTypeToDo('online')") 上线
            el-button.filter-btn(size="small", @click="handleTypeToDo('leaveline')") 下线
      
        .hub-filter-right(v-if='lastupdatetime')    最近一次更新：{{lastupdatetime}}
    
    .hub-result(v-if="resultList.length > 0") 
        .hr-header
            .hrh-item(
        ...
        .hr-content
            .hrc-row(
                ...
                .hrcr-item
                    span(v-if="isOnline(item)") 已上线
                    span.offline(v-else) 未上线
                    span(v-if="isSailing(item)") 特卖中
                    span.not-sailing(v-else-if="item.share_requesting")  申请中
                    span.not-sailing(v-else) 
            
                .hrcr-item
                    .edit
                        button(v-if="isOnline(item)", @click="offline({online: 0, item})") 下线
                        button(v-else, @click="online({online: 1, item})") 上线

                        button(v-if="isSailing(item) || item.share_requesting", @click='cancelSale(item)')   取消特卖
                        button(v-else, @click='applySale(item)') 申请特卖
            ...
            el-dialog.dialog(
                ...
                .cropper-container(
                    ...
                    el-button.cropper-back(@click="cropperBack") 重新选择
                
                .p-count-container
                    span    特卖数量:
                    el-input-number.p-count(
                        v-model="saleCount",
                        @change="inputNumberChange($event)",
                        :min="1",
                        :max="999",
                        label="描述文字"
                    ) 
    ...
    span.hr-remind  *品牌设置为空、或价格为空、或库存为0时，无法上线。
    pagination(
